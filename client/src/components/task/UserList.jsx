...
const UserList = ({ setTeam, team }) => {
  const { data: teamListData, isLoading } = useGetTeamListQuery();
  ...
  useEffect(() => {
    if (!isLoading && teamListData) {
      if (team?.length < 1 && teamListData.length > 0) {
        setSelectedUsers([teamListData[0]]);
      } else {
        setSelectedUsers(team);
      }
    }
  }, [isLoading, teamListData, team]);
        ...
        <div className="relative mt-1">
          <Listbox.Button className="relative w-full cursor-default rounded bg-white pl-3 pr-10 text-left px-3 py-2.5 2xl:py-3 border border-gray-300 sm:text-sm">
            <span className="block truncate">
              {selectedUsers?.filter(Boolean).map((user) => user.name).join(", ") || "Select a user"}
            </span>
